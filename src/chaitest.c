...
const char ver[] = "chaitest ver. 2.8, 01 Sep 2016";
...
//CIO_CAN11 | CIO_CAN29
void chai_open_chans(_u8 bt0, _u8 bt1, _u8 flags)
{
...
void chai_test_transmit(void) 
{
	...
	chai_open_chans(BCI_500K, CIO_CAN11 | CIO_CAN29);
	chai_start_chans();
...
void chai_measure_transmit_series(canmsg_t * send, int send_size)
{
	int sent, ret, sum, err;
	canwait_t cw;
	unsigned long long begint, finisht, difft;
	...
	while (sum < send_size) {
		//printf("sum = %d\n", sum);
		sent = CiTransmitSeries(ch1, &send[sum], send_size - sum, &err);
		if(err < 0 && err != -ECINORES)
			tcall(err, "CiTransmitSeries");
		if (sent == 0) {
			//fulltimes++;
			//printf("tr queue is full %d times\r", fulltimes);fflush(stdout);
			if ((ret = CiWaitEvent(&cw, 1, 1000)) > 0) {
...
canmsg_t rbuf[CIQUE_DEFSIZE_RC];

void init_srbufs(void)
{
	int i, j;
	for (i = 0; i < CIQUE_DEFSIZE_RC; i++) {
		msg_zero(&rbuf[i]);
		...
		sbuf[i].len = 8;
		for (j = 0; j<8; j++)
			sbuf[i].data[j] = (i + j) & 0xFF;
	}
}

void check_rbuf(_u8 rch, int rsize)
{
	int i, j;

	tcall( CiRead(rch, rbuf, rsize), "CiRead failed" );
...
fault:
	...
	("\ntest fail (frame %d was distorted): \n", i);
	if (msg_iseff(&sbuf[i]))
		printf("EFF ");
	else
		printf("SFF ");
	printf("sent id=0x%lx, len=%d, data: ",
		(unsigned long)sbuf[i].id, sbuf[i].len);
	for (j = 0; j < 8; j++)
		printf(" 0x%x", sbuf[i].data[j]);
	printf("\n");
	...
	if (msg_iseff(&rbuf[i]))
		...
		printf("SFF ");
	printf("rcv id=0x%lx, len=%d, data: ",
		(unsigned long)rbuf[i].id, rbuf[i].len);
	for (j = 0; j < 8; j++)
		printf(" 0x%x", rbuf[i].data[j]);
	printf("\n");
	exit_fail();
}
...
void chai_test_measure_transmit(_u8 bt0, _u8 bt1) 
{
	...
	// CiTransmit
	chai_open_chans(bt0, bt1, CIO_CAN11 | CIO_CAN29);
	chai_start_chans();
    // if one of the channel is CAN-bus-USBnp fw ver < 1.4 - complete
    if (check_is_cbunp(ch1, &fwver)) {
        if (fwver < CHAI_VER(0, 1, 4)) {
	...
	check_rbuf(ch2, CIQUE_DEFSIZE_RC);
	chai_close_chans();
	...
	// if one of the channel is CAN-bus-USBnp fw ver < 1.5 - complete
	if (check_is_cbunp(ch1, &fwver)) {
		if (fwver < CHAI_VER(0, 1, 5)) {
			printf
			("channel %d is CAN-bus-USBnp firmware version less then 1.5\n",
				ch1);
			printf
			("This test is not applicable for CAN-bus-USBnp firmware less then 1.5 ... skipped\n");
			goto out;
		}
	}
	if (check_is_cbunp(ch2, &fwver)) {
	...
	printf("Measure TransmitSeries with default transmit threshold: %u\n",
		(_u16)CIQUE_TR_THRESHOLD_DEF);
	init_srbufs();
	chai_measure_transmit_series(sbuf, CIQUE_DEFSIZE_RC);
out:
	chai_close_chans();
